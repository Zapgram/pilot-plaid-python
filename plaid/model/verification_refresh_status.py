# coding: utf-8

"""
    The Plaid API

    The Plaid REST API. Please see https://plaid.com/docs/api for more details.  # noqa: E501

    The version of the OpenAPI document: 2020-09-14_1.394.0
    Generated by: https://openapi-generator.tech
"""

from datetime import date, datetime  # noqa: F401
import decimal  # noqa: F401
import functools  # noqa: F401
import io  # noqa: F401
import re  # noqa: F401
import typing  # noqa: F401
import typing_extensions  # noqa: F401
import uuid  # noqa: F401

import frozendict  # noqa: F401

from plaid import schemas  # noqa: F401


class VerificationRefreshStatus(
    schemas.EnumBase,
    schemas.StrSchema
):
    """NOTE: This class is auto generated by OpenAPI Generator.
    Ref: https://openapi-generator.tech

    Do not edit the class manually.

    The verification refresh status. One of the following:

`"VERIFICATION_REFRESH_STATUS_USER_PRESENCE_REQUIRED"` User presence is required to refresh an income verification.
`"VERIFICATION_REFRESH_SUCCESSFUL"` The income verification refresh was successful.
`"VERIFICATION_REFRESH_NOT_FOUND"` No new data was found after the income verification refresh.
    """


    class MetaOapg:
        enum_value_to_name = {
            "VERIFICATION_REFRESH_STATUS_USER_PRESENCE_REQUIRED": "STATUS_USER_PRESENCE_REQUIRED",
            "VERIFICATION_REFRESH_SUCCESSFUL": "SUCCESSFUL",
            "VERIFICATION_REFRESH_NOT_FOUND": "NOT_FOUND",
        }
    
    @schemas.classproperty
    def STATUS_USER_PRESENCE_REQUIRED(cls):
        return cls("VERIFICATION_REFRESH_STATUS_USER_PRESENCE_REQUIRED")
    
    @schemas.classproperty
    def SUCCESSFUL(cls):
        return cls("VERIFICATION_REFRESH_SUCCESSFUL")
    
    @schemas.classproperty
    def NOT_FOUND(cls):
        return cls("VERIFICATION_REFRESH_NOT_FOUND")
